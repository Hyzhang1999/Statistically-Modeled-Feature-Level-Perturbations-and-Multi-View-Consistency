...
def var_all_case(model, num_classes, patch_size=(128, 128, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
    ...
    print('average metric in {} is {}'.format(dataset_name,avg_dice))
    return avg_dice



def var_part_case(model, num_classes, patch_size=(128, 128, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        with open('../data/LA/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["../data/LA/2018LA_Seg_Training Set/" + item.replace('\n', '') + "/mri_norm2.h5" for item in
...
def test_all_case(model_name, num_outputs, model, image_list, num_classes, patch_size=(128, 128, 80), stride_xy=18,
        ...
        for y in range(0, sy):
            ...
            for z in range(0, sz):
                ...
                with torch.no_grad():
                    ...
                    #print('type of out is:',type(out))
                    y = out[0] if type(out) is tuple else out
                    y = F.softmax(y, dim=1)
